...
- [1. 目录](#目录)
- [2. 前置知识](#前置知识)
    - [客户端向服务端发送请求数据主要包含哪些部分？](#客户端向服务端发送请求数据主要包含哪些部分)
    - [后端如何接受前端发送过来的参数？](#后端如何接受前端发送过来的参数)
        - [Spring Boot 和Spring MVC](#spring-boot-和spring-mvc)
        - [**Java Servlet API**](#java-servlet-api)
    - [后端处理之后返回给前端，前端又怎么去接收这些数据？](#后端处理之后返回给前端前端又怎么去接收这些数据)
        - [JavaScript](#javascript)
        - [Axios](#axios)
        - [jQuery](#jquery)
        - [前端处理HTTP响应:](#前端处理http响应)
            - [注意事项:](#注意事项)
    - [前端向后端发送请求的过程中，参数可以放在哪个部分？](#前端向后端发送请求的过程中参数可以放在哪个部分)
- [3. 序列化](#序列化)
